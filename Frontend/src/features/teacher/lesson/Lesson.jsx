...
import { createPreviewLessonAsync, fetchPreviewLessonAsync, selectPreviewLesson, selectPreviewLessonLoader } from "../../APILibrary/DeitaAPI/deitaSlice";
...
const Lesson = () => {
  ...
  const isDeitaLoading = useSelector(selectPreviewLessonLoader);
  ...
  const uploadCheck = queryParams.get("upload");
  ...
  const onSubmit = () => {
    ...
    if (lessonId) {
      ...
      dispatch(updateLessonAsync(lessonData)).then((result) => {
        ...
        dispatch(createPreviewLessonAsync({ lessonId: currentLessonId }));
        setSubmitted(true);
      });
    } else {
      if (!isSubmitted) {
  ...
  const handlePreviewLessonClick = () => {
    ...
    if (currentLesson._id) {
      if (lessonId) {
        dispatch(fetchPreviewLessonAsync({ lessonId: lessonId }))
      }
      navigate(
  ...
  useEffect(() => {
    const fetchLessonData = async () => {
      if (lessonId) {
        const result = await dispatch(fetchCurrentLessonAsync({ lessonId }));
        ...
        if (uploadCheck === "true") {
          await dispatch(createPreviewLessonAsync({ lessonId }));
        } else {
          await dispatch(fetchPreviewLessonAsync({ lessonId }));
        }
      } else {
        resetForm();
      }
